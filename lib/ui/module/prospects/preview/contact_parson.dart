...
import 'contact_preview_item_card.dart';

class ContactPreviewCard extends BaseStateLessWidget {
  ContactPreviewCard({super.key, required this.contactperson});
  final controller=Get.put(ContactController());
  final List<ContactItemInput> contactperson;
  final isCollapse = false.obs;
  final RxList<ContactPreviewItemCard> contactList = <ContactPreviewItemCard>[].obs;
  ...
  List<Widget> get list {
    for ( int i=0;i<=contactperson.length-1;i++) {
      contactList.add(ContactPreviewItemCard(contactDetail: contactperson[i],index:i));
    }
    return contactList;
  }
